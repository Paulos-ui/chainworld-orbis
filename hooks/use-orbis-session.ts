...
export function useOrbisSession(
  ...
  const startWithPrompt = (nextPrompt: string) => {
    setPrompt(nextPrompt);
    return runAction(() => startGeneration(null, nextPrompt));
  };

  const steerWithPrompt = (nextPrompt: string) => {
    setPrompt(nextPrompt);
    return runAction(async () => {
      if (!nextPrompt.trim()) throw new Error("A world event needs a prompt.");
